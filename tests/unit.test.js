...
import { parseNativePair, nativeStats, toNativeData } from '../src/native/anymaker-data.js';
import { createNode, moveNode, moveNodeAndMerge, mergeNodes, removeNode, removeEdge, removePlate, createEdge, createBeam, splitEdge, createPlate, triangulatePlate, validateTopologyState } from '../src/editor/topology.js';
import * as THREE from 'three';
import { reflectGeometry } from '../src/assets/geometry-ops.js';
import { GRID_SIZE, GRID_DIVISIONS, cameraBuildFrame, projectBuildPoint, resolveBeamPoint, beamMeasurements, createBeamMesh } from '../src/editor/construction-view.js';
...
test('beam meshes keep XY diagonal side faces parallel to the XY plane', () => {
  ...
  for (const offset of [...axisEnds, planar, new THREE.Vector3(3, 4, 5)]) {
    ...
    assert.ok(Math.abs(beam.scale.y - start.distanceTo(end)) < 1e-9);
    assert.ok(beam.localToWorld(new THREE.Vector3(0, -.5, 0)).distanceTo(start) < 1e-9);
    assert.ok(beam.localToWorld(new THREE.Vector3(0, .5, 0)).distanceTo(end) < 1e-9);
    ...
    if (offset === planar) {
      ...
      const side = [new THREE.Vector3(-.5, -.5, .5), new THREE.Vector3(.5, -.5, .5), new THREE.Vector3(.5, .5, .5)].map(point => beam.localToWorld(point));
      assert.ok(side.every(point => Math.abs(point.z - side[0].z) < 1e-9));
    }
