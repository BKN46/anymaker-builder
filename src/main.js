...
import { History, LIMIT, project, validateDocument, toIntermediateXml } from './editor/document.js';
...
$('#app').innerHTML = '<header class="topbar"><div class="brand"><span>ANY</span>MAKER / BUILDER</div><span class="status" id="save-status" role="status">正在加载定义…</span><nav class="top-actions"><button id="library-btn">导入本地模型</button><button id="new-btn">新建</button><button id="undo-btn">撤销</button><button id="redo-btn">重做</button><button id="save-btn">保存工程</button><button id="load-btn">打开工程</button><button id="export-btn" class="primary">中间格式 XML</button></nav></header>' +
  ...
  '<section id="viewport" aria-label="三维建造视口"><div class="view-controls"><button data-view="iso">等距</button><button data-view="top">顶视</button><button data-view="front">前视</button><button id="fit-btn">聚焦 F</button></div><div class="notice">研究原型：真实静态 Mesh / 中性材质；未完成视觉 1:1。XML 不是已验证的游戏存档。</div><div class="hud"><span class="badge" id="object-count">0 个组件</span><span class="badge" id="cursor-pos">工作平面 Y = 0</span></div></section>' +
...
const tools = [['select', '选择', 'V'], ['place', '放置', 'P'], ['erase', '删除', 'E'], ['translate', '移动', 'G'], ['rotate', '旋转', 'R'], ['scale', '缩放', 'S']];
for (const [id, name, key] of tools) {
  const button = document.createElement('button');
  button.className = 'tool'; button.dataset.tool = id;
  const icons = { select: '↖', place: '＋', erase: '⌫', translate: '✥', rotate: '⟳', scale: '⤢' };
...
function snapshot() {
  return objects.map(o => ({ id: o.userData.id, type: o.userData.type,
    ...
    scale: Object.fromEntries(axes.map(a => [a, o.scale[a]])) }));
}
function commit() {
  // TransformControls may cross zero when scaling; keep the project domain valid.
  if (selected) axes.forEach(a => selected.scale[a] = THREE.MathUtils.clamp(selected.scale[a], .001, 100));
  history.commit(snapshot()); refresh();
}
function refresh() {
...
async function createObject(data) {
  ...
  object.userData = { ...object.userData, id: data.id, type: data.type };
...
async function place(point) {
  ...
  const object = await createObject({ id: crypto.randomUUID(), type: selectedType, position: { x: 0, y: 0, z: 0 }, rotation: { x: 0, y: 0, z: 0 }, scale: { x: 1, y: 1, z: 1 } });
...
function undo() { const value = history.peekUndo(); if (value) transact(async () => { await restore(value); history.cursor--; status('已撤销'); }); }
function redo() { const value = history.peekRedo(); if (value) transact(async () => { await restore(value); history.cursor++; status('已重做'); }); }
...
function inspect() {
  ...
  metadata.textContent = def.id + ' · ' + object.userData.reason + (object.userData.vertices ? ' · ' + object.userData.vertices + ' 顶点 / ' + object.userData.triangles + ' 三角形' : '') + ' · 动态部件 ' + (def.meshes_dynamic?.length || 0) + '（尚未装配）';
...
function currentProject() { return validateDocument(project(snapshot()), catalog.index); }
...
$('#file-input').onchange = e => {
  ...
  transact(async () => {
    ...
    const document = validateDocument(JSON.parse(await file.text()), catalog.index);
    await restore(document.objects); commit(); fit(); status('工程已加载');
  });
};
...
window.addEventListener('keydown', e => {
  ...
  if (key === 'delete' || key === 'backspace') { e.preventDefault(); remove(selected); }
  if (key === 'f') fit();
