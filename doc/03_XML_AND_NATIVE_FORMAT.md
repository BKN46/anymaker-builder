...
当前导入投影将存在的 `origin` 作为局部原点，并将非零 `dir` 归一化为局部 Y（平面法线）；局部 X 为世界 X 在该平面上的投影，局部 Z 为 X × Y。组件位置和原生列主序旋转矩阵都会组合该网格帧，车辆关联锚点也在组合后按多体约束枢轴求差。当前已根据 `hinge_knuckle.constraint_position` 处理该组件的枢轴；`tow_hitch` 的 surface/logic 位置是端口定义，未作为约束枢轴替用。每个子载具的全部已记录锚点须求得同一偏移，不一致会中止投影，不能平均后伪造位置。斜面子网格会产生非整数世界格坐标，编辑器以 `nativeProjected` 标记保留这些精确投影值；这是一项当前样本驱动的渲染/受限回写实现，尚非游戏坐标语义或游戏加载兼容性的最终证据。
