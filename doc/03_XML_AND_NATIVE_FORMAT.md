...
| position | objects[].position | 编辑器世界坐标；手动对象每轴为 8 cm 整数格，`nativeProjected` 原生子网格投影可保留小数 |
...
当前导入投影将存在的 `origin` 作为局部原点，并将非零 `dir` 归一化为局部 Y（平面法线）；局部 X 为世界 X 在该平面上的投影，局部 Z 为 X × Y。组件位置和原生列主序旋转矩阵都会组合该网格帧，车辆关联锚点也在组合后求差，不再从跨网格连接线路估算平移。斜面子网格会产生非整数世界格坐标，编辑器以 `nativeProjected` 标记保留这些精确投影值；这是一项当前样本驱动的渲染/受限回写实现，尚非游戏坐标语义或游戏加载兼容性的最终证据。
